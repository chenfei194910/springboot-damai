...
public class DataSourceAopInService implements PriorityOrdered {
	 ...
	 * " or execution(* com.fei.springboot.service..*.query*(..))") public void
	 * setReadDataSourceType() { //如果已经开启写事务了，那之后的所有读都从写库读
	 * if(!DataSourceType.write.getType().equals(DataSourceContextHolder.
	 * getReadOrWrite())){ DataSourceContextHolder.setRead(); }
	 * 
	 * }
	 * 
	 * @Before("execution(* com.fei.springboot.service..*.insert*(..)) " +
	 * " or execution(* com.fei.springboot.service..*.update*(..))" +
	 * " or execution(* com.fei.springboot.service..*.add*(..))") public void
	 * setWriteDataSourceType() { DataSourceContextHolder.setWrite(); }
	 */
